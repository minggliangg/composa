...
- Add package scripts (run with Bun) for development and production build. The WASM build hook may be a documented placeholder until Phase 07, but it must not make the Phase 01 application unusable.
...
- The development server starts with `bun run dev` and serves the application.
...
Run `bun run dev`, open the printed localhost URL, and resize the browser: the top bar and the three editor regions should remain visibly distinct with no console error.
